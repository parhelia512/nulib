...
bool filterNamespace(string namespace, string[] ignored) {
    foreach(ins; ignored) {
...
auto getNamespaces(const ref Metadata db, string[] ignored)
{
    ...
    return chain(defSet, refSet).filter!(a => a.length > 0 && a.filterNamespace(ignored)).array.sort.uniq;
}
...
void dumpEnumFieldConst(std.stdio.File f, Field field, bool isStruct, string typeText) {    

    // Handle being set to const value.
    bool isConstNull = field.constant.isNull;
    if (!isConstNull) {
        ...
        dumpConstant(f, field.constant.get.value);
...
void dumpFieldCollection(std.stdio.File f, Field[] flds, bool wasOne)
{
    ...
    if (flds.length == 1)
    {
        ...
        dumpEnumFieldConst(f, flds[0], isStruct, typeText);          
        f.writeln(";");
    }
    else
    {        
        ...
        {            
            ...
            dumpEnumFieldConst(f, fx, isStruct, typeText);
            f.writeln(",");
        }
        f.writeln("}");
    }
...
void writeDocComment(std.stdio.File f, string header) {
    f.writeln("/**");
    foreach(line; header.splitLines()) {
        f.writeln("    "~line);
    }
    f.writeln("*/");
}
...
int main(string[] args)
{
    ...
    enum usage = 
        "Usage: \n" ~
        "  wind --meta <filename> --core <filename> --coremod a.b.c\n" ~
        "  wind --meta <filename> --core <filename> --coremod a.b.c --out <dir> --ignore <filename> --replace <filename>";
