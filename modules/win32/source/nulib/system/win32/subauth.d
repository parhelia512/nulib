 ...
 */
module nulib.system.win32.subauth;


import nulib.system.win32.ntdef, nulib.system.win32.windef;
...
alias PSTRING = STRING*;
+/

alias SAM_HANDLE = HANDLE;
alias PSAM_HANDLE = SAM_HANDLE*;

struct OLD_LARGE_INTEGER {
    ULONG LowPart;
    LONG HighPart;
}
alias POLD_LARGE_INTEGER = OLD_LARGE_INTEGER*;
...
alias PCYPHER_BLOCK = CYPHER_BLOCK*;

struct CLEAR_BLOCK {
    CHAR[CLEAR_BLOCK_LENGTH] data = 0;
}
alias PCLEAR_BLOCK = CLEAR_BLOCK*;

struct LM_OWF_PASSWORD {
    CYPHER_BLOCK[2] data;
}
alias PLM_OWF_PASSWORD = LM_OWF_PASSWORD*;

struct USER_SESSION_KEY {
    CYPHER_BLOCK[2] data;
}
alias PUSER_SESSION_KEY = USER_SESSION_KEY*;

alias LM_CHALLENGE = CLEAR_BLOCK;
alias PLM_CHALLENGE = LM_CHALLENGE*;

alias NT_OWF_PASSWORD = LM_OWF_PASSWORD;
alias PNT_OWF_PASSWORD = NT_OWF_PASSWORD*;
alias NT_CHALLENGE = LM_CHALLENGE;
alias PNT_CHALLENGE = NT_CHALLENGE*;

struct LOGON_HOURS {
    USHORT UnitsPerWeek;
    PUCHAR LogonHours;
}
alias PLOGON_HOURS = LOGON_HOURS*;

struct SR_SECURITY_DESCRIPTOR {
    ULONG Length;
    PUCHAR SecurityDescriptor;
}
alias PSR_SECURITY_DESCRIPTOR = SR_SECURITY_DESCRIPTOR*;
...
alias PUSER_ALL_INFORMATION = USER_ALL_INFORMATION*;
align:
...
alias PMSV1_0_VALIDATION_INFO = MSV1_0_VALIDATION_INFO*;
...
alias PNETLOGON_LOGON_IDENTITY_INFO = NETLOGON_LOGON_IDENTITY_INFO*;
...
alias PNETLOGON_INTERACTIVE_INFO = NETLOGON_INTERACTIVE_INFO*;
...
alias PNETLOGON_GENERIC_INFO = NETLOGON_GENERIC_INFO*;
...
alias PNETLOGON_NETWORK_INFO = NETLOGON_NETWORK_INFO*;
...
alias PNETLOGON_SERVICE_INFO = NETLOGON_SERVICE_INFO*;
