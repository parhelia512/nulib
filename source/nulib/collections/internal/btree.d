/**
    ...
    Authors:   
        ...
        Luna Nielsen
*/
module nulib.collections.internal.btree;
import numem;
...
struct BTree(
    ...
    alias less = (a, b) => a < b, // must be strict inequality
...
@nogc:
    ...
    */
    ...
    /**
        ...
        Returns:
            $(D true) if there's zero elements, 
            $(D false) otherwise.
    */
    bool empty() const {
        return _count == 0;
    ...
    /**
        ...
        Returns:
            $(D true) if the insertion took place, $(D false) otherwise.

        WARNING:
            Inserting duplicate keys when duplicates are not supported
...
private:
    ...
    inout(Node)* findNode(inout(Node)* x, K key, out int index) inout {
        int i = 0;
        while (i < x.numKeys && _less(cast(K)x.kv[i].key, cast(K)key))
            i += 1;

        // Like in Phobos' Red Black tree, use !less(a,b) && !less(b,a) instead
        // of opEquals.

        if (i < x.numKeys && !_less(cast(K)key, cast(K)x.kv[i].key)) {
            index = i;
            return x;
        } else {
            if (x.isLeaf)
    ...
    static struct KeyValue {
        ...
        this(ref return scope inout(KeyValue) rhs) {
            this.key = cast(K)rhs.key;
            this.value = cast(V)rhs.value;
        }
    }

    debug (btree) public void display() {
